...
class SearchMovieDelegate extends SearchDelegate<Movie?> {
  ...
  StreamController<List<Movie>> debounceStream = StreamController.broadcast();
    ...
    return [
      StreamBuilder(
        ...
        builder: (context, snapshot) {
          return snapshot.data ?? false ? SpinPerfect(
        duration: const Duration(seconds: 20),
        spins: 10,
        infinite: true,
          // animate: query.isNotEmpty,
          child:
              IconButton(onPressed: () => query = '', icon: Icon(Icons.refresh_rounded)))
              :  FadeIn(
          animate: query.isNotEmpty,
          child:
  ...
  Widget _buildResultsAndSuggestions() {
      ...
      stream: debounceStream.stream,
      builder: (context, snapshot) {
        // if(!snapshot.hasData)

        // final temMovies = debounceStream.stream.last;
